...
const ItemTemplate = React__default["default"].memo((props) => {
    const { primaryColor, period, changePeriod, changeInputText, updateFirstDate, dayHover, changeDayHover, hideDatepicker } = React.useContext(DatepickerContext);
    // Functions
    const getClassName = React.useCallback(() => {
...
const Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    const activeDateData = React.useCallback((day) => {
        ...
        if ((dayjs__default["default"](fullDay).isSame(period.start) && dayjs__default["default"](fullDay).isSame(period.end))) {
            ...
            // @ts-ignore
            className = ` ${BG_COLOR["500"][primaryColor]} text-white font-medium ${(dayjs__default["default"](fullDay).isSame(dayHover) && !period.end) ? "rounded-full" : "rounded-l-full"}`;
        }
        else if (dayjs__default["default"](fullDay).isSame(period.end)) {
            // @ts-ignore
            className = ` ${BG_COLOR["500"][primaryColor]} text-white font-medium ${(dayjs__default["default"](fullDay).isSame(dayHover) && !period.start) ? "rounded-full" : "rounded-r-full"}`;
        }
        return {
